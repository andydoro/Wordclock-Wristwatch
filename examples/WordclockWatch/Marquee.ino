...
void mode_marquee(uint8_t action) {
  ...
  if(action != ACTION_NONE) {
    ...
    if(marqueeSubmode == MARQUEE_SUBMODE_TIME) {
      ...
      if(!h24) str[len++] = (theTime.hour() < 12) ? SYM_AM : SYM_PM;
    } else {
      loadDigits(theTime.year() - 2000, DIGIT_YEAR0);
      loadDigits(theTime.month()      , DIGIT_MON0);
      loadDigits(theTime.day()        , DIGIT_DAY0);
